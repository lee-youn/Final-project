...
class TripleHeadVideoClassifier(nn.Module):
    def __init__(self, n_dv, n_pl, n_ov):
        super().__init__()
...
@torch.no_grad()
def evaluate(model, loader, device, epoch=None, epochs=None, save_confmat=True):
    model.eval()
    y_dv, y_pl, y_ov = [], [], []
    p_dv, p_pl, p_ov = [], [], []
    ...
    # 혼동행렬 저장 (옵션)
    if save_confmat:
        plot_confmat(y_dv, p_dv, LABELS["dashcam_vehicle_info"], "DV Confusion Matrix", "cm_dv.png","val/dv/confmat")
        plot_confmat(y_pl, p_pl, LABELS["accident_place_feature"], "Place Confusion Matrix", "cm_place.png","val/dv/confmat")
        plot_confmat(y_ov, p_ov, LABELS["other_vehicle_info"], "OV Confusion Matrix", "cm_ov.png","val/dv/confmat")
...
def main():
    ...
    ap.add_argument("--out_ckpt", type=str, default="./checkpoints/3dcnn.pth")  # (원하면 유지)
    ...
    if args.mode == "train":
        ...
        model = TripleHeadVideoClassifier(NCLS["dashcam_vehicle_info"],
                                          ...
                                          NCLS["other_vehicle_info"]).to(device)
        ...
        for ep in range(1, args.epochs+1):
            ...
            # === 2) Validate ===
            val = evaluate(model, val_loader, device, epoch=ep, epochs=args.epochs)
            log_dict = {
                "epoch": ep,
                "train/loss": tr_loss,
                ...
                "val/place/acc": val["place"]["acc"],
                "val/place/f1_macro": val["place"]["f1_macro"],
                "val/place/balanced_acc": val["place"]["balanced_acc"],
            ...
            if val["exact_match"] > best_exact:
                ...
                ckpt_path = os.path.join(args.save_dir, f"best_exact_ep{ep}.pth")
                torch.save({
        ...
        plot_summary(val, out_path="results_metrics.png")
        end_time = time.time()
        ...
        model = TripleHeadVideoClassifier(NCLS["dashcam_vehicle_info"],
                                          NCLS["accident_place_feature"],
                                          NCLS["other_vehicle_info"]).to(device).eval()
        ...
        plot_summary(val, out_path="results_metrics_validation.png")
