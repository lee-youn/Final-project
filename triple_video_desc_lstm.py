...
def train_one_epoch(model, loader, opt, device, fp16=False, epoch=1, epochs=1, log_interval=10, global_step_offset=0):
    ...
    for step, batch in enumerate(pbar, start=1):
        ...
        scaler.scale(loss).backward()
        torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0)   # ★ 추가
        scaler.step(opt); scaler.update()
...
def evaluate(model, loader, device, epoch=None, epochs=None, save_confmat=True, backbone="r3d18"):
    ...
    if save_confmat:
        plot_confmat(y_dv, p_dv, LABELS["dashcam_vehicle_info"], "DV Confusion Matrix", f"cm_dv_{backbone}_lstm.png","val/dv/confmat")
        plot_confmat(y_ov, p_ov, LABELS["other_vehicle_info"], "OV Confusion Matrix", f"cm_ov_{backbone}_lstm.png","val/ov/confmat")
...
def plot_summary(metrics: Dict, out_path="results_metrics_lstm.png"):
...
def main():
    ...
    wandb.init(project="video-desc-lstm", config=vars(args))
    ...
    if args.mode == "train":
        ...
        for ep in range(1, args.epochs+1):
            ...
            if f1_mean > best_f1:
                best_f1 = f1_mean
                os.makedirs(args.save_dir, exist_ok=True)
                ckpt_path = os.path.join(args.save_dir, f"best_f1_ep{ep}_{args.backbone}_lstm.pth")
                torch.save({
                    "model": model.state_dict(),
                    "epoch": ep,
                    "best_f1": best_f1
        ...
        plot_summary(val, out_path=f"results_metrics_{args.backbone}_lstm.png")
        end_time = time.time()
        ...
        plot_summary(val, out_path=f"results_metrics_validation_{args.backbone}_lstm.png")
