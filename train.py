...
# TRACK_JSON_DIR_TRAIN = "data/tracks/Preprocessing/train"
# TRACK_JSON_DIR_VAL   = "data/tracks/Preprocessing/val"
...
# W&B 설정
WANDB_PROJECT = "accident-caption-multimodal"
...
class AccidentVideoDataset(Dataset):
    def __init__(self, meta_path, video_dir, tokenizer, num_frames=NUM_FRAMES, 
                 frame_size=FRAME_SIZE, meta_drop_prob=0.0):
        import csv, json
        self.rows = []
    ...
    def __getitem__(self, idx):
        ...
        # 필요 시, 정답과 겹치지 않는 부가 정보만 포함 가능
        place = r.get('accident_place_feature')
        place = str(place).strip() if place is not None else ""
        dashcam = r.get('dashcam_vehicle_info')
        dashcam = str(dashcam).strip() if dashcam is not None else ""
        other = r.get('other_vehicle_info')
        other = str(other).strip() if other is not None else ""
        
        # 정답(description)와 겹치지 않는 안전한 context만 src_text에 추가
        # context_parts = []
        # if place: context_parts.append(f"location: {place}")
        # if dashcam: context_parts.append(f"dashcam vehicle: {dashcam}")
        # if other: context_parts.append(f"other vehicle: {other}")
        ...
        try:
            ...
            tgt_encoding = self.tok(
                ...
                truncation=True,
                max_length=64,
                padding=False,           # ❗️ max_length 패딩 금지 (배치에서 패딩)
                return_tensors="pt",
            )
            labels = tgt_encoding["input_ids"].squeeze(0)            # [L_tgt] (아직 -100 처리 X)
