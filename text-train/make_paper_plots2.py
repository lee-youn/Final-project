...
"""
make_paper_plots.py  (updated)
- Top-K context cosine bar (latent v vs template-conditioned phrase vectors)
- Local PCA map of {v} ∪ Top-K
- Prints P@K, Δ(mean_imp - mean_dist), permutation p-value
- NEW: AUROC / Average Precision (AP)
- NEW: (optional) save context vectors as .npy for follow-up training
...
def plot_bar_topk(phrases, scores, groups, out_png, out_pdf, title):
    ...
    plt.ylabel("cosine(latent v, context phrase)")
    plt.title(title)
...
def plot_local_pca(v, means, labels, groups, out_png, out_pdf, title):
    ...
    # latent v
    star = plt.scatter(XY[0,0], XY[0,1], s=160, marker="*", edgecolor="k", c=GREEN, zorder=5, label="latent vector v")
    # simple legend (proxy for groups)
...
def main():
    ...
    ap.add_argument("--save_context_vecs", action="store_true", help="save per-phrase context vectors (.npy) for later training")
    args = ap.parse_args()
    ...
    # (optional) save context vectors for later contrastive training
    if args.save_context_vecs:
        np.save(os.path.join(args.out_dir, f"{args.save_prefix}_context_vecs.npy"),
                np.stack([cv.cpu().numpy() for cv in ctx_vecs], axis=0))
        with open(os.path.join(args.out_dir, f"{args.save_prefix}_context_meta.csv"), "w", newline="") as f:
            w = csv.writer(f); w.writerow(["phrase","group"])
            for p,g in zip(phrases, groups):
                w.writerow([p,g])
        print("[Saved] raw context vectors (.npy) and meta CSV")
    ...
    # optional AUROC/AP
    if _HAVE_SK:
        labels = np.array([1 if g=="important" else 0 for g in groups])
        scores = np.array(ctx_scores)
        mask = np.isfinite(scores)
        try:
            auc = roc_auc_score(labels[mask], scores[mask])
            ap  = average_precision_score(labels[mask], scores[mask])
            print(f"[AUC] AUROC = {auc:.3f}, AP = {ap:.3f}")
        except Exception as e:
            auc = float("nan"); ap = float("nan")
            print(f"[AUC] skipped ({e})")
    else:
        auc = float("nan"); ap = float("nan")
        print("[AUC] scikit-learn not available; skip AUROC/AP")
    ...
    with open(os.path.join(args.out_dir, f"{args.save_prefix}_caption_stats.txt"), "w") as f:
        ...
        if _HAVE_SK and np.isfinite(auc):
    ...
    with open(os.path.join(args.out_dir, f"{args.save_prefix}_knn_pca_coords.csv"), "w", newline="") as f:
        ...
        w.writerow(["<latent>","latent", XY[0,0], XY[0,1]])
