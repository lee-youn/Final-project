...
_COLLISION_SPLIT_RE = re.compile(r'\bthe collision\b', re.IGNORECASE)

def trim_caption(s: str) -> str:
    if not isinstance(s, str):
        return s
    parts = _COLLISION_SPLIT_RE.split(s, maxsplit=1)
...
def load_and_prepare_dataframe(data_path: str) -> pd.DataFrame:
    ...
    df["generated_caption"] = df["generated_caption"].astype(str).map(trim_caption)
    df["generated_caption"] = df["generated_caption"].str.strip()
    df = df[df["generated_caption"].str.len() > 0]
...
def train_model(
    ...
    fp16: bool = False,
    grad_clip: float = 5.0,
    target_basis: float = 10.0,   # 40/60 -> 4/6
):
    """
    target_basis: labels range [0, target_basis], default 10.0
    """
    wandb.init(project=os.environ.get("WANDB_PROJECT", "fault_ratio_regression-final"),
               config={"lr": lr, "batch_size": batch_size, "epochs": epochs,
                       "model": model_name, "target_basis": target_basis,
                       "train_rows": len(train_df) if train_df is not None else None,
                       "eval_rows": len(eval_df) if eval_df is not None else None})

    # --- 데이터 준비 분기 ---
    if train_df is None or eval_df is None:
        if df is None:
            raise ValueError("train_model: train_df/eval_df 또는 df 중 하나는 반드시 제공해야 합니다.")
        train_df, eval_df = train_test_split(df, test_size=0.2, random_state=42)
