...
with gr.Blocks(title="Video-LLaVA Chatbot") as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=2):
            ...
            session = gr.State(_default_session()) # 비디오/예측 캐시

            btn_ratio = gr.Button("Watch the video and give a split-liability ratio")
            btn_who   = gr.Button("Watch the video and give a who is at fault.")

            user_msg = gr.Textbox(
                label="Your message",
                placeholder="(버튼을 눌러도 되고, 직접 영어로 입력해도 됩니다)",
            )
        ...
        btn_ratio.click(
            ...
            outputs=[state, session, chatbot, user_msg],  # ✅ state와 chatbot 동시 갱신
        ).then(
            ui_turn,
            inputs=[
                ...
                style, num_frames, frame_size, temperature, sampling,
                use_soft, interp_mode, soft_gain,
                gr.State(True)  # skip_user_echo=True
            ],
            outputs=[chatbot, session, state],  # (history, session, history) 순서에 맞춤
        ).then(
            lambda: gr.update(value=""),  # 또는 lambda: ""
            inputs=None,
            outputs=[user_msg],
        )
        ...
        btn_who.click(
            ...
            outputs=[state, session, chatbot, user_msg],  # ✅ 동일
        ).then(
            ui_turn,
            inputs=[
                state, session, user_msg, video, video_name, model_id,
        ...
        send_btn.click(
            ...
            inputs=[
                ...
                style, num_frames, frame_size, temperature, sampling,
                use_soft, interp_mode, soft_gain,
                gr.State(False)  # ✅ 직접 입력은 여기서 유저 버블 추가
            ],
            outputs=[chatbot, session, state],
        ).then(
